...
ny20Key = process.env.NY20;
mapsKey = process.env.MAPS;
...
var timer = function() {
    async function jsonParse (satellite) {
        const res = await fetch(`https://www.n2yo.com/rest/v1/satellite/positions/${satID[satellite]}/52.070499/4.300700/0/1/&apiKey=${ny20Key}`);
        console.log(res);
    }
};
...
//         const meta = await fetch(`https://maps.googleapis.com/maps/api/streetview/metadata?size=600x300&location=${location}&pitch=90&key=${mapsKey}`)
...
//             const sky = await fetch(`https://maps.googleapis.com/maps/api/streetview?size=600x600&location=${foundSats}&pitch=90&key=${mapsKey}`);
//             const skyDest = fs.createWriteStream(`sky/${timestamp}.jpg`);
//             const land = await fetch(`https://maps.googleapis.com/maps/api/staticmap?center=${foundSats}&zoom=13&size=600x600&maptype=satellite&key=${mapsKey}`);
